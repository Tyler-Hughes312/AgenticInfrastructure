...
export default function DriftPanel({ state }: { state: any }) {
  ...
  return (
    <div className="text-sm space-y-2">
      ...
      {loading ? (
        <p className="text-sm text-charcoal-muted">Calculating...</p>
      ) : !drift ? (
        <p className="text-sm text-charcoal-muted">No drift computed yet.</p>
      ) : (
        <div className="text-sm space-y-2">
          ...
          {drift.flags?.length ? (
            <ul className="pt-2 border-t border-charcoal-border space-y-1">
              {drift.flags.map((f: string, i: number) => (
                <li key={i} className="text-charcoal-muted">
                  {f}
                </li>
              ))}
            </ul>
